create or replace table fivetran_esteem_provide_staging.fulfillment_orders_temp as
(
  with order_items as
  (
  ...
  (
      select  
        ...
        case 
          when trim(internal_notes) = '' then null 
          else internal_notes 
        end internal_notes,
        items,
        st.shipment_tags,
        ...
        case 
          when trim(fc.email_address) != '' then lower(trim(fc.email_address)) 
          end customer_email,
        o.organization_id,
        ilt.ikea_location_name,
        case 
          when ends_with(ilt.ikea_location_name,'STO') then 'Store'
          when ends_with(ilt.ikea_location_name,'CDC') then 'CDC' 
        end ikea_location_type,
